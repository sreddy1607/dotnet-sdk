        ...
        public override async Task WatchAsync(CancellationToken shutdownCancellationToken)
        {
            ...
            if (!Context.Options.NonInteractive)
            {
                ...
                _console.KeyPressed += (key) =>
                {
                    ...
                    if ((key.Modifiers & modifiers) == modifiers && key.Key == ConsoleKey.R && forceRestartCancellationSource is { } source)
                    {
                        ...
                        Context.Reporter.Output(source.IsCancellationRequested ? "Restart in progress." : "Restart requested.", emoji: "🔄");
                        source.Cancel();
                    }
                };
            }
            ...
            for (var iteration = 0; !shutdownCancellationToken.IsCancellationRequested; iteration++)
            {
                ...
                try
                {
                    ...
                    using var rootRunningProject = await projectLauncher.TryLaunchProcessAsync(rootProjectOptions, rootProcessTerminationSource, build: true, iterationCancellationToken);
                    ...
                    Task<FileItem[]?> fileSetWatcherTask;
                    var rootProcessTask = rootRunningProject.RunningProcess;
                    ...
                    while (true)
                    {
                        ...
                        var finishedTask = await Task.WhenAny(rootProcessTask, fileSetWatcherTask).WaitAsync(iterationCancellationToken);
                        if (finishedTask == rootProcessTask)
                        {
                            // Cancel the iteration, but wait for a file change before starting a new one.
                            iterationCancellationSource.Cancel();
                            break;
                    ...
                    // Wait for dotnet to exit. Child processes will be terminated upon CompilationHandler disposal.
                    await Task.WhenAll(rootProcessTask, fileSetWatcherTask);
                }
                catch (OperationCanceledException) when (!shutdownCancellationToken.IsCancellationRequested)
                {
                    // Ctrl+R:
                    if (forceRestartCancellationSource.IsCancellationRequested)
                    {
                        _console.Clear();
                    }
                }
