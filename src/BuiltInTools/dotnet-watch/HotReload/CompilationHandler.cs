...
namespace Microsoft.DotNet.Watcher.Tools
{
    internal sealed class CompilationHandler : IAsyncDisposable
    {
        ...
        private async ValueTask DisplayResultsAsync(WatchHotReloadService.Updates updates, CancellationToken cancellationToken)
        {
            ...
            Display(DiagnosticSeverity.Error);
            Display(DiagnosticSeverity.Warning);

            void Display(DiagnosticSeverity severity)
            {
                foreach (var diagnostic in updates.Diagnostics)
                {
                    ...
                    else if (diagnostic.Id == "ENC1005")
                    {
                        ...
                        adjustedSeverity = DiagnosticSeverity.Info;
                        display += " Expected if a source file is updated that is linked to project whose build is not up-to-date.";
                    }
                    ...
                    if (adjustedSeverity == DiagnosticSeverity.Error)
                    {
                        _reporter.Error(display);
                    }
                    else if (adjustedSeverity == DiagnosticSeverity.Warning)
                    {
                        _reporter.Warn(display, emoji: "⚠");
                    }
                    else
                    {
                        _reporter.Verbose(display);
                    }

                    diagnosticsToDisplay.Add(display);
                }
            }
