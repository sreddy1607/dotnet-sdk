...
public class RuntimeProcessLauncherTests(ITestOutputHelper logger) : DotNetWatchTestBase(logger)
{
    ...
    public async Task UpdateAndRudeEdit(TriggerEvent trigger)
    {
        ...
        var waitingForChanges = new SemaphoreSlim(initialCount: 0);
        reporter.RegisterAction(MessageDescriptor.WaitingForChanges, () => waitingForChanges.Release());
        ...
        var changeHandled = new SemaphoreSlim(initialCount: 0);
        reporter.RegisterAction(MessageDescriptor.HotReloadChangeHandled, () => changeHandled.Release());

        var sessionStarted = new SemaphoreSlim(initialCount: 0);
        reporter.RegisterAction(MessageDescriptor.HotReloadSessionStarted, () => sessionStarted.Release());
    ...
    public async Task UpdateAppliedToNewProcesses(bool sharedOutput)
    {
        ...
        var updatesApplied = new SemaphoreSlim(initialCount: 0);
        reporter.RegisterAction(MessageDescriptor.UpdatesApplied, () => updatesApplied.Release());
    ...
    [Theory]
    [CombinatorialData]
    public async Task HostRestart(bool updateDependency)
    {
        var testAsset = TestAssets.CopyTestAsset("WatchAppMultiProc", identifier: updateDependency.ToString())
            .WithSource();
        ...
        var restartNeeded = new SemaphoreSlim(initialCount: 0);
        // TODO:
        // reporter.RegisterAction(..., () => restartNeeded.Release());
        ...
        else
        {
            ...
            // Host received updates and is being restarted:
